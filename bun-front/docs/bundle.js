...
// share/lib/useFetched.ts
var cache = new Map;
var useFetched = (apiMethod, initial, cached = false) => {
  const [data, setData] = h2(initial);
  y2(() => {
    if (cached && cache.has(apiMethod)) {
      setData(cache.get(apiMethod));
      return;
    }
    apiMethod().then((data2) => {
      cache.set(apiMethod, data2);
      setData(data2);
    });
  }, []);
  return data;
...
var UserTable = () => {
  ...
  const last10 = useFetched(api_default.last10, []);
  y2(() => {
    const uid = new URL(document.location.href).searchParams.get("uid") || 552926829;
    api_default.postByUser(+uid).then(setData);
  }, []);
...
// pages/top-users-table.tsx
var TopUsersTable = () => {
  const top = useFetched(api_default.top, []);
