...
const controllers: Record<string, Controller> = {
  postsbyuser: ([uid]: string[], params: Record<string, string>) => db.query.postsByUser.all({ uid }),
  settoken: ([token]) => {
...
Bun.serve({
  async fetch(request, server) {
    ...
    const [_, api, controller, ...paths] = url.pathname.split('/')
    const params: any = {}
    for (let [k, v] of url.searchParams.entries()) params[k] = v;
    let res = 'not found', status = 404
    if (api === 'api') {
