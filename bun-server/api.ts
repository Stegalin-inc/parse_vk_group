// получить токен
// https://oauth.vk.com/authorize?client_id=1&display=page&redirect_uri=http://vk.com/callback&scope=friends,wall,offline&response_type=token&v=5.131&state=123456
...
const vkApi = <T>(method: string, data: any = {}) => {
  ...
  return fetch(url, { body: new URLSearchParams(data), method: "POST" }).then((x) =>
    x.json().then((x) => ({ res: x.response, err: x.error }))
  ) as Promise<Res<T>>;
};
...
export default {
  ...
  posts: (stage = 0, pass = 25, cnt = 100) =>
    vkApi<PostRaw>("execute", {
      code: `
      var stage=${stage};
      var pass = ${pass}; 
      var cnt = ${cnt};
